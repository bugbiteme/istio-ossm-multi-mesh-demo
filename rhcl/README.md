...
11. **Verify DNSPolicy status**
    ...
    (Note that your hosted zone in Route 53 now has several records generated by the DNS Policy, and will be removed when you delete the policy)
...
after applying you will get a response 
```json
{
  "error": "Forbidden",
...
Since RHCL generates DNS records for your hosted zone, it is important to clean up your environment when tearing it down

1. *** Delete DNSPolicy ***

Deleting the DNSPolicy will remove kuadrant generated records from Route 53
...
When you delete a TLSPolicy, RHCL's `cert-manager` integration will remove the `Certificate` resources it created. `cert-manager` will then delete the associated `CertificateRequest` and `Order` objects.

```bash
oc -n ingress-gateway delete tlspolicy prod-gateway-tls
```

3. *** Delete AuthPolicies and RateLimitPolicies to allow all traffic ***
...
4. *** Revert Gateway and HTTPRoute without domain name ***

Once you delete the DNS policy, you can still access your application via Gateway API, but using the LB FQDN/IP address
