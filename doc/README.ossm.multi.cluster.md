# Multi-cluster OSSM demo — step-by-step
...
Wait for the Tempo stack to be ready on both clusters (Tempo takes time to provision; errors are expected until it stabilizes):
...
### 7.4 Expose services through the east-west gateways
...
### 7.5 Enable cross-cluster endpoint discovery
...
### 7.6 Ingress gateway deployment
...
Run the same commands on the **west** cluster:

```bash
oc --context="${CTX_WEST}" get secret tempo-sample-signing-ca -n tracing-system \
    -o jsonpath='{.data.tls\.crt}' | base64 -d > certs/ca.crt 
oc --context="${CTX_WEST}" create secret generic cacert --from-file=ca.crt=certs/ca.crt -n istio-system    
```
...
### 9.4 Validate access to API via gateway

```bash
curl -so - -w "%{http_code}\n" http://${GATEWAY_URL}/api/v1/products/0/ratings | jq
```

### 9.5 Load generator scripts (web and API; can run simultaneously)
...
### 9.6 Service mesh testing (chaos engineering)

#### 9.6.1 Fault injection (east cluster `ratings`)
